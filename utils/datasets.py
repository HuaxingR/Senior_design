...
from utils.augmentations import Albumentations, augment_hsv, copy_paste, letterbox, mixup, random_perspective, \
    linearletterbox, LinearAlbumentations
...
################################################################3
############ add a dataloader for linear ###############
def create_linear_dataloader(path, clsnum, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False,
                               cache=False,
                               pad=0.0,
...
def linear_img2label_paths(img_paths):
    # Define label paths as a function of image paths
    sa, sb = os.sep + 'images' + os.sep, os.sep + 'labels' + os.sep  # /images/, /labels/ substrings
    return [sb.join(x.rsplit(sa, 1)).rsplit('.', 1)[0] + '.txt' for x in img_paths]
...
##############################################################
################ add an image and label loader ##################
class LoadLinearImagesAndLabels(Dataset):
    ...
    def __init__(self, path, cls_num=20, img_size=640, batch_size=16, augment=False, hyp=None, rect=False,
        ...
        self.stride = stride
        self.path = path
        self.albumentations = LinearAlbumentations() if augment else None
        ...
        # Check cache
        self.label_files = linear_img2label_paths(self.img_files)  # labels
